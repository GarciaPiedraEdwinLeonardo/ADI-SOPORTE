// src/pages/tickets/tecnico/TicketsTecnico.jsx
...
import styles from './TicketsTecnico.module.css'
    ...
    return (
        ...
        >
            ...
            <div className={styles.cardBadges}>
                ...
                <span className={styles.badge} style={{ color: priority.color, background: priority.bg }}>
                    {priority.label}
                </span>
                {sla && (
            ...
            <div className={styles.cardFooter}>
                <span className={styles.cardDate}>Creado {formatDateShort(ticket.created_at)}</span>
            </div>
        </article>
    )
}
...
export default function TicketsTecnico() {
    ...
    const filtered = tickets.filter(t => {
        ...
        return matchStatus && matchPrio && matchArea
    })
    ...
    // ── Paginación ─────────────────────────────────────────────────────────────
    useEffect(() => { setPage(1) }, [filterStatus, filterPrio, filterArea])
    const totalPages = Math.ceil(filtered.length / PAGE_SIZE)
    const paginated  = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE)

    const hasFilters = filterStatus !== 'all' || filterPrio !== 'all' || filterArea !== 'all'
                            ...
                            <div className={styles.titleRow}>
                                ...
                                <div>
                                    ...
                                    <p className={styles.pageSubtitle}>
                                        {tickets.length} ticket{tickets.length !== 1 ? 's' : ''} asignados
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    {!loading && !error && filtered.length === 0 && (
                        <div className={styles.emptyState}>
                            <div className={styles.emptyIcon}>
                                {hasFilters ? <MdFilterList size={30} /> : <MdCheckCircle size={30} />}
                            </div>
                            <h3 className={styles.emptyTitle}>
                                {hasFilters ? 'Sin resultados' : '¡Sin tickets pendientes!'}
                            </h3>
                            ...
                            {hasFilters && (
                                <button className={styles.btnClearFiltersLg} onClick={clearFilters}>
                                    Limpiar filtros
                                </button>
                            )}
                        </div>
                            ...
                            <div className={styles.grid}>
                                {paginated.map(ticket => (
                                    <TicketCard
                                        key={ticket.id}
                                        ticket={ticket}
                                        onClick={() => navigate(`/tickets/tecnico/${ticket.id}`)}
                                    />
                                ))}
                            </div>
